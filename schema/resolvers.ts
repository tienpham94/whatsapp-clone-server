...
export default ({
  ...
  Query: {
    ...
    chats: () =>
      chats.filter(chat => chat.listingMemberIds.includes(currentUser)),
  ...
  Chat: {
    name: chat =>
      chat.name
        ...
        : users.find(
            user =>
              user.id ===
              chat.allTimeMemberIds.find(
                (userId: string) => userId !== currentUser
              )
          )!.name,
    picture: chat =>
      chat.name
    ...
    actualGroupMembers: chat =>
      users.filter(
        user =>
          chat.actualGroupMemberIds &&
          chat.actualGroupMemberIds.includes(user.id)
      ),
    admins: chat =>
      ...
      chat.messages.filter(
        (message: Message) =>
          ...
          message.recipients.find(
            (recipient: Recipient) =>
              recipient.userId === currentUser && !recipient.readAt
          )
      ).length,
  ...
  Message: {
    chat: message => chats.find(chat => message.chatId === chat.id) || null,
    sender: message => users.find(user => user.id === message.senderId) || null,
    holders: message =>
      users.filter(user => message.holderIds.includes(user.id)),
    ownership: message => message.senderId === currentUser
  },
  Recipient: {
    user: recipient => users.find(user => recipient.userId === user.id) || null,
    message: recipient => {
      const chat = chats.find(chat => recipient.chatId === chat.id)
      return chat
        ? chat.messages.find(message => recipient.messageId === message.id) ||
            null
        : null
    },
    chat: recipient => chats.find(chat => recipient.chatId === chat.id) || null
  }
